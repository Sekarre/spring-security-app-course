...
public class Google2faFilter extends GenericFilterBean {
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException, ServletException {
        ...
        StaticResourceRequest.StaticResourceRequestMatcher staticResourceRequestHandler
                = PathRequest.toStaticResources().atCommonLocations();

        if (urlIs2fa.matches(request) || urlResources.matches(request) ||
                staticResourceRequestHandler.matcher(request).isMatch()) {
        ...
        if (authentication != null && !authenticationTrustResolver.isAnonymous(authentication)) {
            ...
            if (authentication.getPrincipal() != null && authentication.getPrincipal() instanceof User) {
                ...
                if (user.getUseGoogle2fa() && user.isGoogle2faRequired()) {
